...
//            ->remove(Crud::PAGE_NEW, Action::SAVE_AND_ADD_ANOTHER)
    ...
    public function export(Request $request)
    {
        ...
        foreach ($contrats as $contrat) {
            $data[] = $contrat->getExportDataPromo();
        }

        return $this->csvService->export($data, 'export_promo_' . date_create()->format('d-m-y') . '.csv');
    }

    public function import(Request $request, EntityManagerInterface $em)
    {
        $form = $this->createFormBuilder()
            ->add('ImportPromo', FileType::class, [
                'required' => true,
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            /** @var UploadedFile */
            $file = $form->get('ImportPromo')->getData();
            ...
            if (($handle = fopen($file->getPathname(), "r")) !== false) {
                ...
                while (($data = fgetcsv($handle, 0, ',')) !== false) {
                    $count++;
                    $entity = new Promo();
                    $classe = $em->getRepository(Classe::class)->findOneBy(['nom' => $data[1]]);


                    $entity->setNom($data[0]);
                    $entity->addClasse($classe);
